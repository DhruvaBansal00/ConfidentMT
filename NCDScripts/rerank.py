...
import rerank_generate, rerank_score_bw, rerank_score_lm, rerank_options, rerank_utils


def score_target_hypo(args, a, b, c, lenpen, target_outfile, hypo_outfile, score_outfile, lm_outfile, fw_outfile, bw_outfile, write_hypos, normalize):

    print("lenpen", lenpen, "weight1", a, "weight2", b, "weight3", c, "target_outfile", target_outfile, "hypo_outfile", hypo_outfile, "lm_outfile", lm_outfile)
    ...
    for shard_id in range(len(bitext1_lst)):
        ...
        for key in range(len(gen_keys)):
            if args.prefix_len is None:
                ...
                if args.sacrebleu:
                    scorer.add_string(gen_output.no_bpe_target[gen_keys[key]], hypo_lst[key])
                else:
                        ...
                        with open(fw_outfile, 'a') as f:
                            with open(bw_outfile, 'a') as b:
                                for key in range(len(ordered_hypos)):
                                    t.write("Target = " + ordered_targets[key])
...
def match_target_hypo(args, target_outfile, hypo_outfile, score_outfile, lm_outfile, fw_outfile, bw_outfile,):
    ...
    if len(args.weight1) == 1:
        res = score_target_hypo(args, args.weight1[0], args.weight2[0],
                                ...
                                hypo_outfile, score_outfile, lm_outfile, fw_outfile, bw_outfile, True, args.normalize)
        rerank_scores = [res]
    else:
        print("avoiding pool. Total num in args.weight1 = ", len(args.weight1))
        # with Pool(32) as p:
        ...
        for i in range(len(args.weight1)):
            rerank_scores.append(score_target_hypo(args, args.weight1[i], args.weight2[i], args.weight3[i],
                                        args.lenpen[i], target_outfile, hypo_outfile, score_outfile, lm_outfile,
                                        fw_outfile, bw_outfile, False, args.normalize))
...
def load_score_files(args):
    ...
    for shard_id in shard_ids:
        ...
        gen_output = rerank_utils.BitextOutputFromGen(predictions_bpe_file, bpe_symbol=args.bitext_remove_bpe,
        ...
        if rerank1_is_gen:
            ...
            bitext1 = rerank_utils.BitextOutput(score1_file, args.backwards1, args.right_to_left1,
                                                args.bitext_remove_bpe, args.prefix_len, args.target_prefix_frac,
        ...
        if args.language_model is not None:
            lm_res1 = rerank_utils.LMOutput(lm_score_file, args.lm_dict, args.prefix_len,
                                            args.lm_remove_bpe, args.target_prefix_frac)
        else:
            lm_res1 = None
...
def rerank(args):
    ...
    for shard_id in shard_ids:
        ...
        if args.write_hypos is None:
            ...
            write_scores = pre_gen+"/hypos_scores"
            write_lm = pre_gen+"/lm_scores"
            write_fw = pre_gen+"/fw_scores"
            write_bw = pre_gen+"/bw_scores"
        else:
            write_targets = args.write_hypos+"_targets" + args.gen_subset
            write_hypos = args.write_hypos+"_hypos" + args.gen_subset
            write_scores = args.write_hypos+"_scores" + args.gen_subset
            write_lm = args.write_hypos+"_lms"+args.gen_subset
            write_fw = args.write_hypos+"_fws"+args.gen_subset
            write_bw = args.write_hypos+"_bws"+args.gen_subset

    if args.all_shards:
        write_targets += "_all_shards"
        write_hypos += "_all_shards"
        write_scores += "_all_shards"
        write_lm += "_all_shards"
        write_fw += "_all_shards"
        write_bw += "_all_shards"

    best_lenpen, best_weight1, best_weight2, best_weight3, best_score = \
        match_target_hypo(args, write_targets, write_hypos, write_scores, write_lm, write_fw, write_bw)
